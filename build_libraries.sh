...
build_library() {
    ...
    if [ -n "$configure_opts" ]; then
        if [ $lib_name == "readline" ]; then
            "$src_dir/configure" --host=$CROSS_HOST --prefix="$install_dir" --enable-shared --disable-static CPPFLAGS="-I$CROSS_BASE/install/ncurses/include" CFLAGS="$CFLAGS -I$CROSS_BASE/install/ncurses/include" LDFLAGS="-L$CROSS_BASE/install/ncurses/lib" LIBS="$CROSS_BASE/install/ncurses/lib/libncursesw.so $CROSS_BASE/install/ncurses/lib/libtinfow.so"
        else
